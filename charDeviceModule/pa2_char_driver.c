...
ssize_t pa2_char_driver_read (struct file *pfile, char __user *buffer, size_t length, loff_t *offset)
{
	...
	if(maxBytesToRead > length)
	{
	 maxBytesToWrite =  length;
	}
	else
	{
	 maxBytesToWrite = maxBytesToRead;
	}
	...
	bytesRead = maxBytesToRead - copy_to_user(buffer, kptr + *offset,  maxBytesToWrite);
...
ssize_t pa2_char_driver_write (struct file *pfile, const char __user *buffer, size_t length, loff_t *offset)
{
	...
	if(maxBytesToRead > length)
        {
         maxBytesToWrite =  DEVICE_BUFFER - *offset;
        }
        else
        {
         maxBytesToWrite = maxBytesToRead;
	...
	bytesWritten = maxBytesToRead - copy_from_user(kptr + *offset, buffer,  maxBytesToWrite);
...
static int pa2_char_driver_init(void)
{
	...
	printk(KERN_ALERT "charDriver init has been called");
	/* register the device */
...
static void pa2_char_driver_exit(void)
{
	/* print to the log file that the exit function is called.*/
	 printk(KERN_ALERT "charDriver exit has been called");
